...
public class AttachGameObjectsToParticles : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        {
            m_Instances[i].transform.localScale = new Vector3(m_Particles[i].startSize, m_Particles[i].startSize,
                m_Particles[i].startSize);
            if (i < count)
            {
                if (worldSpace)
            ...
            m_Instances[i].intensity = (m_Particles[i].remainingLifetime / 3f);
        }
    }
}
